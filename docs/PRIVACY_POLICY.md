...
| `https://raw.githubusercontent.com/mahdibland/V2RayAggregator/master/sub/sub_merge.txt` (GitHub) — the pre-installed **"Public Free Servers"** subscription (v1.0.3) | once on first launch, then like any other subscription until you delete it or turn its auto-update off | HTTPS GET of the list; the servers in it are run by unknown third parties — traffic you send through them is visible to their operators. The app never connects to them on its own. |
