...
| `RECEIVE_BOOT_COMPLETED` | ℹ present, dependency-owned | Attributed by the manifest-merger blame report to `androidx.work:work-runtime` (its `RescheduleReceiver`). The app declares none, has no boot receiver and no boot auto-connect; the permission is what lets WorkManager re-arm the 12-hourly subscription refresh after a reboot. Normal permission, no Play declaration. Approval marker + CI enforcement: see `docs/RELEASE.md` §Permissions. |
