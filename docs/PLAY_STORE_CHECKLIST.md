...
| `RECEIVE_BOOT_COMPLETED` | ✅ app declaration removed / ℹ still merged | The app has no boot receiver and no boot auto-connect (always-on VPN is the system's). The permission remains in the merged manifest because `androidx.work` declares it for `RescheduleReceiver` (re-arms periodic subscription refresh after reboot). Normal permission, no Play declaration needed. |
