...
| `RECEIVE_BOOT_COMPLETED` | ℹ present, dependency-owned | Attributed by the manifest-merger blame report to `androidx.work:work-runtime` (its `RescheduleReceiver`). The app declares none itself; it is what lets WorkManager re-arm the 12-hourly subscription refresh after a reboot, and since v1.0.1 also serves the opt-in (default OFF) "Connect on boot" `BootReceiver`, which acts only with existing VPN consent. Normal permission, no Play declaration. Approval marker + CI enforcement: see `docs/RELEASE.md` §Permissions. |
