...
| Allow LAN (v1.0.1, default off) | `lanProxy`, `lanProxyPort` (1024–65535, default 2080) | `inbounds += { type: mixed, tag: lan-in, listen: "0.0.0.0", listen_port }` — **unauthenticated** SOCKS5/HTTP listener; the UI shows an "open proxy" warning while on. Traffic from it follows the same route rules as TUN. An out-of-range port is a validation error and never opens a listener. |
