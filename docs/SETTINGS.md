...
| Connect on boot (v1.0.1, default off) | `autoConnectOnBoot` — **UI-only, not in `CoreStartOptions`** | `core/vpn/BootReceiver` (not exported; `BOOT_COMPLETED`/`LOCKED_BOOT_COMPLETED`) → `BootConnectPolicy.decide(enabled, VpnService.prepare()==null, selectedProfileId, lastActiveProfileId, manager.state.isActive)` → `ConnectionManager.connect(id)` — i.e. the normal FGS start path. Skips (logged) when the switch is off, consent is missing, no profile exists or the tunnel is already up (always-on / STICKY restart). No consent dialog is ever shown from the receiver. |
