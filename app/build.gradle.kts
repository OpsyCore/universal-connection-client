...
/** Official Telegram channel (v1.0.3). Override with UCC_VIP_URL / -PvipUrl; set to "" to hide the card. */
val DEFAULT_VIP_URL = "https://t.me/UCC_VPN_OFFICIAL"

/** VIP card URL: env UCC_VIP_URL → gradle property `vipUrl` → "" (hidden). Validated so a typo cannot ship a dead button. */
fun vipUrl(): String {
    val raw = (System.getenv("UCC_VIP_URL") ?: (project.findProperty("vipUrl") as String?) ?: DEFAULT_VIP_URL).trim()
