...
/**
 * Affiliate / VIP subscription card (v1.0.3). Rendered **only** when `BuildConfig.VIP_URL` is non-blank
 * (build-time `UCC_VIP_URL` / `vipUrl`); there is no placeholder link, so a build without a real URL shows
 * nothing. Opens the URL in the system handler (Telegram for t.me / tg:// links). Theme colours, string
...
fun VipCard(modifier: Modifier = Modifier, url: String = BuildConfig.VIP_URL) {
    ...
    Card(
        ...
        Row(Modifier.fillMaxWidth().padding(16.dp), verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.SpaceBetween) {
            Icon(Icons.Filled.WorkspacePremium, contentDescription = null, tint = MaterialTheme.colorScheme.tertiary)
