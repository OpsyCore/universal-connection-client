...
public object ShareLinkExporter {
    ...
    public fun exportAllBase64(profiles: List<ConnectionProfile>): String =
        Base64.getEncoder().encodeToString(exportAll(profiles).toByteArray(Charsets.UTF_8))
    ...
    private fun vmessJson(p: ConnectionProfile, a: Authentication.Vmess): String {
        ...
        return "vmess://" + Base64.getEncoder().encodeToString(obj.toString().toByteArray(Charsets.UTF_8))
    }

    private fun shadowsocks(p: ConnectionProfile, a: Authentication.Shadowsocks): String {
        val userInfo = Base64.getUrlEncoder().withoutPadding().encodeToString("${a.method}:${a.password}".toByteArray(Charsets.UTF_8))
        val q = LinkedHashMap<String, String>()
    ...
    /** RFC-3986 percent-encoding (URLEncoder is form-encoding: fix `+` and keep `~`). */
    private fun enc(s: String): String = URLEncoder.encode(s, "UTF-8").replace("+", "%20").replace("%7E", "~")
}
