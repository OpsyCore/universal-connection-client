...
class ShareLinkExporterTest {
    ...
    @Test fun `vmess exports as v2rayN base64 JSON and round-trips`() {
        ...
        assertTrue(Base64Codec.decodeStrict(exported.removePrefix("vmess://")).decodeToString().startsWith("{"))
    ...
    @Test fun `wireguard round-trips`() {
        val priv = Base64Codec.encode(ByteArray(32) { 1 })
        val pub = Base64Codec.encode(ByteArray(32) { 2 })
        roundTrip("wireguard://${UrlCodec.encode(priv)}@1.2.3.4:51820?publickey=${UrlCodec.encode(pub)}&address=10.0.0.2/32,fd00::2/128&reserved=1,2,3&mtu=1280#wg")
    }
    ...
    @Test fun `exportAll emits one link per line and base64 variant decodes to it`() {
        ...
        assertEquals(text, Base64Codec.decodeStrict(ShareLinkExporter.exportAllBase64(ps)).decodeToString())
    }
}
