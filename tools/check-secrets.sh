...
if git grep -n -I -i -E -- "$pat" -- ':!*/test/*' ':!*/androidTest/*' ':!docs/*' ':!*.md' ':!tools/check-secrets.sh' | cut -d: -f1,2 | grep . ; then report "credential-looking literal"; else echo "   none"; fi

echo ">> URLs with embedded credentials (outside tests/docs/parsers)"
if git grep -n -I -E -- '(https?|vless|vmess|trojan|ss|hysteria2?|tuic|socks5?)://[^/@[:space:]"]+:[^/@[:space:]"]+@[a-zA-Z0-9.-]+\.[a-z]{2,}' -- ':!*/test/*' ':!docs/*' ':!*.md' ':!core/config/src/main/*' ':!tools/check-secrets.sh' | cut -d: -f1,2 | grep . ; then report "URL with credentials"; else echo "   none"; fi
